...
from .abstract_predictor import PredictionMethod
...
class Sizey(PredictionMethod):
    ...
    def __init__(
        ...
        self.randomForestPredictor = RandomForestPredictor(
            workflow_name="Test", task_name="Test", err_metr=error_metric
        )
        self.knnPredictor = KNNPredictor(
            workflow_name="Test", task_name="Test", err_metr=error_metric
        )
        ...
        print("y_train")
        print(y_train)
        # For multi-feature support, calculate the first feature value at max memory index
        max_mem_idx = np.where(y_train == self.max_mem)[0][0]
        self.max_input_size = X_train.values[
            max_mem_idx
        ]  # Store all features, not just first
        print("max_input_size")
        print(self.max_input_size)
        self.min_mem = min(y_train)[0]
        self.X_full = X_train.values
        self.y_full = y_train
        # For multiple features, use Spearman correlation with first feature as proxy
        # or calculate correlation with target for each feature
        if X_train.shape[1] == 1:
            self.kedall_corr = stats.kendalltau(
                self.X_full.flatten(), self.y_full.flatten()
            )
        else:
            # Use first feature for correlation calculation (can be improved)
            self.kedall_corr = stats.kendalltau(
                self.X_full[:, 0], self.y_full.flatten()
            )
    ...
    def predict(
        self, X_test: pd.Series, y_test: int, user_estimate
    ) -> Tuple[float, float]:
        ...
        # Update correlation calculation for multi-feature case
        if self.X_full.shape[1] == 1:
            self.kedall_corr = stats.kendalltau(
                self.X_full.flatten(), self.y_full.flatten()
            )
        else:
            # Use first feature for correlation calculation
            self.kedall_corr = stats.kendalltau(
                self.X_full[:, 0], self.y_full.flatten()
            )
        ...
        y_pred_softmax = (
            prediction_lin * (np.exp(beta * raq_lin) / sum_raq_softmax)
            + prediction_nn * (np.exp(beta * raq_nn) / sum_raq_softmax)
            + prediction_rf * (np.exp(beta * raq_rf) / sum_raq_softmax)
            + prediction_knn * (np.exp(beta * raq_knn) / sum_raq_softmax)
        )

        y_pred_softmax_offset = (
            offset_lin * (np.exp(beta * raq_lin) / sum_raq_softmax)
            + offset_nn * (np.exp(beta * raq_nn) / sum_raq_softmax)
            + offset_rf * (np.exp(beta * raq_rf) / sum_raq_softmax)
            + offset_knn * (np.exp(beta * raq_knn) / sum_raq_softmax)
        )
        ...
        if memToPredict < 0:
            return (
                self.min_mem * self.default_offset,
                self.min_mem * self.default_offset,
            )
        else:
    ...
    def handle_underprediction(
        ...
        next_pred = self._get_next_pred_for_underpred(
            input_size, predicted, user_estimate
        )

        if next_pred > actual_memory:
            self.failures.append(
                {
                    "retry": self._get_next_pred_for_underpred(
                        input_size, predicted, user_estimate
                    ),
                    "peak_memory": actual_memory,
                    "retry_strategy": self.error_strategy,
    ...
    def _calculate_raq_score(self, X_test, y_test) -> Tuple[float, float, float, float]:
        accuracy_lin, accuracy_nn, accuracy_rf, accuracy_knn = (
            self._calculate_accuracy_score()
        )
        efficiency_lin, efficiency_nn, efficiency_rf, efficiency_knn = (
            self._calculate_efficiency_score(X_test)
        )
        ...
        if offset_strategy == OFFSET_STRATEGY.MED_ALL:
            ...
            if len(list(filter(self._check_gt0, prediction_error))) > 0:
                return statistics.median(
                    list(filter(self._check_gt0, prediction_error))
                )
            else:
                return default_offset
    ...
    def _get_next_pred_for_underpred(
        self, input_size: float, prediction: float, user_estimate: float
    ) -> float:
        print(input_size)
        print(self.max_input_size)
        ...
        if self.error_strategy == ERROR_STRATEGY.DOUBLE:
            ...
            if self.max_mem <= prediction:
                return prediction * 2
            elif (self.kedall_corr.correlation > 0.25) & (
    ...
    def _select_dynamic_offset_failures(self, prediction_error, predictor):
        ...
        for offset_strat in [
            ...
            y_hat = (
                y_hat
                + self._get_offset(
                    offset_strat, self.default_offset, prediction_error, predictor
                )
                * y_hat
            )
    ...
    def get_number_subModels(self) -> dict[str, int]:
        return {
            "lin": self.lin_counter,
            "nn": self.nn_counter,
            "rf": self.rf_counter,
            "knn": self.knn_counter,
            "max": self.max_counter,
            "softmax": self.softmax_counter,
        }
