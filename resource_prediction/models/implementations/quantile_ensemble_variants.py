...
class XGBXGBQuantileEnsemble(BasePredictor):
    """
    An ensemble regressor combining two XGBoost models with different quantile configurations.
    
    Uses a conservative model (higher quantile, deeper trees, fewer estimators) and
    an aggressive model (lower quantile, shallower trees, more estimators).
    ...
    def __init__(
        ...
        conservative_quantile: float = 0.98,
        conservative_n_estimators: int = 200,
        conservative_max_depth: int = 8,
        conservative_lr: float = 0.03,
        # Aggressive model parameters (lower quantile, shallower trees, more estimators)
        aggressive_quantile: float = 0.90,
        aggressive_n_estimators: int = 500,
        aggressive_max_depth: int = 5,
        aggressive_lr: float = 0.08,
        **kwargs
    ):
        """
        Initialize the XGBoost + XGBoost Quantile Ensemble Predictor.
        
        Args:
            alpha: Base quantile level (used as fallback, default: 0.95)
            safety: Safety multiplier applied to final predictions (default: 1.05)
            random_state: Random state for reproducibility
            conservative_quantile: Quantile level for conservative model
            conservative_n_estimators: Number of estimators for conservative model
            conservative_max_depth: Max depth for conservative model
            conservative_lr: Learning rate for conservative model
            aggressive_quantile: Quantile level for aggressive model
            aggressive_n_estimators: Number of estimators for aggressive model
            aggressive_max_depth: Max depth for aggressive model
            aggressive_lr: Learning rate for aggressive model
            **kwargs: Additional parameters (ignored gracefully)
        """
        self.alpha = alpha
        self.safety = safety
        ...
        # Conservative XGBoost model (higher quantile, deeper trees, fewer estimators)
        self.conservative_xgb = xgb.XGBRegressor(
            objective="reg:quantileerror",
            quantile_alpha=conservative_quantile,
            n_estimators=conservative_n_estimators,
            max_depth=conservative_max_depth,
            learning_rate=conservative_lr,
            n_jobs=1,
            random_state=random_state
        )
        
        # Aggressive XGBoost model (lower quantile, shallower trees, more estimators)
        self.aggressive_xgb = xgb.XGBRegressor(
            objective="reg:quantileerror",
            quantile_alpha=aggressive_quantile,
            n_estimators=aggressive_n_estimators,
            max_depth=aggressive_max_depth,
            learning_rate=aggressive_lr,
            n_jobs=1,
            random_state=random_state
        )
...
class XGBXGBStandardQuantileEnsemble(BasePredictor):
    """
    ...
    def set_params(self, **params) -> 'XGBXGBStandardQuantileEnsemble':
        """Set model parameters."""
        for key, value in params.items():
            if hasattr(self, key):
                setattr(self, key, value)
