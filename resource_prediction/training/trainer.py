...
from resource_prediction.reporting import (
    calculate_allocation_categories,
...
class Trainer:
    ...
    def _train_with_defaults(self):
        ...
        if self.model_families:
            families_to_train = [
                (name, metadata)
                for name, metadata in families_to_train
    ...
    def run_evaluation_from_files(self):
        ...
        for file_path in [
            self.config.REGRESSION_RESULTS_CSV_PATH,
            self.config.CLASSIFICATION_RESULTS_CSV_PATH,
        ]:
            if file_path.exists():
    ...
    def _update_and_save_champion_results(
        self, new_results_df: pd.DataFrame, path: Path
    ):
        """
        ...
        final_df = (
            combined_df.loc[best_indices].sort_values("model").reset_index(drop=True)
        )
    ...
    def _evaluate_single_champion(
        self, study, config, X_train, y_train, X_test, y_test, save_model: bool
    ):
        """
        ...
        y_train_gb, y_test_gb = (
            y_train[config.TARGET_COLUMN_PROCESSED],
            y_test[config.TARGET_COLUMN_PROCESSED],
        )
        ...
        if task_type == "classification" and confidence_threshold is not None:
            ...
            if family_name == "sizey_regression":
                alloc = model.predict(X_test_fs, y_test_gb)
            else:
    ...
    def _evaluate_and_report(self, studies, force_evaluate_all: bool = False):
        ...
        regr_studies = [
            ...
            if self.config.MODEL_FAMILIES[self._get_family_name_from_study(s)]["type"]
            == "regression"
        ]
        class_studies = [
            s
        ...
        if should_eval_all:
            ...
            if self.task_type_filter != "classification" and regr_studies:
                models_to_evaluate.append(min(regr_studies, key=lambda s: s.best_value))
        ...
        for study in models_to_evaluate:
            family_name = self._get_family_name_from_study(study)
            print(f"  - {family_name.upper()} (CV Score: {study.best_value:.4f})")

        print("\nEvaluating models...")
        regr_results, class_results, all_model_stats = [], [], []
        for study in tqdm(
            models_to_evaluate, total=len(models_to_evaluate), desc="Evaluating Models"
        ):
            try:
        ...
        if should_eval_all and not regr_df.empty and not class_df.empty:
            ...
            if not all_results.empty:
                ...
                print(
                    f"Score vs prediction time plot saved to {self.config.SCORE_TIME_PLOT_PATH}"
                )
