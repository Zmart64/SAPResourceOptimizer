...
class Trainer:
    ...
    def run_simple_training(self):
        """
        Simple training workflow for --train option:
        - Train models on training split with default parameters
        - Evaluate once on holdout split  
        - Report holdout scores only
        - No CV, no champion files, no complex evaluation pipeline
        """
        if self.X_train is None:
            print("Exiting due to missing data.")
            return
        ...
        for family_name, metadata in families_to_train:
            ...
            print(f"Training {family_name} on training split...")
            
            try:
                holdout_score = self._train_and_evaluate_simple(family_name, metadata)
                results.append({'model': family_name, 'holdout_score': holdout_score})
                print(f"  {family_name}: Holdout Score = {holdout_score:.4f}")
        ...
        if results:
            print(f"\nTraining complete. Evaluated {len(results)} model(s) on holdout split.")
            print("\nFinal Results:")
            for result in sorted(results, key=lambda x: x['holdout_score']):
                print(f"  {result['model']}: {result['holdout_score']:.4f}")
        else:
            print("\nNo models were successfully trained.")

    def _train_and_evaluate_simple(self, family_name, metadata):
        """
        Train a single model on the training split and evaluate on holdout split.
        Returns the holdout score (business score for regression, accuracy for classification).
        """
        ...
        # Prepare features
        use_quant_feats = default_params.pop("use_quant_feats", True)
        base_features = self.config.BASE_FEATURES + (self.config.QUANT_FEATURES if use_quant_feats else [])
        X_train_fs = self.X_train[base_features]
        X_test_fs = self.X_test[base_features]
        y_train_gb = self.y_train[self.config.TARGET_COLUMN_PROCESSED]
        y_test_gb = self.y_test[self.config.TARGET_COLUMN_PROCESSED]
        
        # Encode features if needed
        if base_model != 'quantile_ensemble':
            X_train_fs = pd.get_dummies(X_train_fs, drop_first=True, dummy_na=False).astype(float)
            X_test_fs = pd.get_dummies(X_test_fs, drop_first=True, dummy_na=False).astype(float)
        ...
        if task_type == 'regression':
            if base_model == 'quantile_ensemble':
                ...
                gb_params = {'n_estimators': default_params["gb_n_estimators"],
                           'max_depth': default_params["gb_max_depth"], 
                           'learning_rate': default_params["gb_lr"], 'verbose': 0}
                xgb_params = {'n_estimators': default_params["xgb_n_estimators"],
                            'max_depth': default_params["xgb_max_depth"], 
                            'learning_rate': default_params["xgb_lr"]}
                model = QuantileEnsemblePredictor(
                    alpha=alpha, safety=default_params["safety"], 
                    gb_params=gb_params, xgb_params=xgb_params)
            elif base_model == 'xgboost':
                model = xgb.XGBRegressor(**default_params, random_state=self.config.RANDOM_STATE)
            elif base_model == 'lightgbm':
                model = lgb.LGBMRegressor(**default_params, random_state=self.config.RANDOM_STATE, verbose=-1)
            else:
            ...
            metrics = self._allocation_metrics(predictions, y_test_gb.values)
            return self._business_score(metrics)
            
        else:  # classification
            n_bins = default_params.pop("n_bins")
            strategy = default_params.pop("strategy")
            
            # Create bins based on training data
            min_val, max_val = y_train_gb.min(), y_train_gb.max()
            if strategy == 'quantile':
                try:
                    _, bin_edges = pd.qcut(y_train_gb, q=n_bins, retbins=True, duplicates='drop')
                except ValueError:
            ...
            y_train_binned = pd.cut(y_train_gb, bins=bin_edges, labels=False, include_lowest=True, right=True)
            
            # Fix parameter names for specific models
            if 'lr' in default_params:
                default_params['learning_rate'] = default_params.pop('lr')
            
            # Create and train model
            if base_model == 'xgboost':
                model = xgb.XGBClassifier(**default_params, random_state=self.config.RANDOM_STATE)
            elif base_model == 'lightgbm':
                model = lgb.LGBMClassifier(**default_params, random_state=self.config.RANDOM_STATE, verbose=-1)
            elif base_model == 'random_forest':
                model = RandomForestClassifier(**default_params, random_state=self.config.RANDOM_STATE)
            elif base_model == 'logistic_regression':
                model = LogisticRegression(**default_params, max_iter=1000, random_state=self.config.RANDOM_STATE)
            else:
                raise ValueError(f"Unknown classification model: {base_model}")
            
            # Fit and predict
            model.fit(X_train_fs, y_train_binned)
            pred_classes = model.predict(X_test_fs).astype(int)
            predictions = bin_edges[np.minimum(pred_classes + 1, len(bin_edges) - 1)]
