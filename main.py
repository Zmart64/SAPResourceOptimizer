...
def main(args):
    ...
    if not args.skip_preprocessing and (
        args.run_search or args.train_default or args.evaluate_only
    ):
    ...
    if args.run_all_qe_models:
        ...
        if model_families:
            ...
            model_families = list(
                sorted(set(Config.MODEL_FAMILIES.keys()).union(all_qe_models))
            )
    else:
        ...
        if model_families is None:
            ...
            model_families = [
                m for m in all_models if m not in experimental_qe_ensembles
            ]
